...
class BernoulliQTLScan(QTLScan):
    def __init__(self, outcome, covariates, X, Q0, Q1, S0):
        super(BernoulliQTLScan, self).__init__(X)
        self._cache_compute_null_model = LRUCache(maxsize=1)
        self._cache_compute_alt_models = LRUCache(maxsize=1)
        self._noccurrences = outcome
    ...
    def _compute_null_model(self, progress):
        outcome = self._noccurrences
        covariates = self._covariates
        Q0, Q1 = self._Q0, self._Q1
        S0 = self._S0

        ep = BernoulliEP(outcome, covariates, Q0=Q0, Q1=Q1, S0=S0)
